...
try:
    while True:
        ...
        #contours, hierarchy = cv2.findContours(color_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2:]
        print("color image")
        contours, hierarchy = cv2.findContours(color_frame, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2:]
        print("color frame")
        ...
        bg_removed = np.where((depth_image_3d > clipping_distance) | (depth_image_3d <= 0), grey, color_image)
